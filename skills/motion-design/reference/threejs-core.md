...
Three.js is mid-migration from `WebGLRenderer` toward `WebGPURenderer`, but it hasn't stabilized enough to take over as the default recommendation. The official manual is explicit about this: `WebGPURenderer` "is still in an experimental state," and `WebGLRenderer` "is still maintained and the recommended choice for pure WebGL 2 applications." `WebGPURenderer` does fall back to WebGL2 automatically on browsers/devices without WebGPU support, so it's positioned as a superset rather than a parallel track, and new large features are landing on the WebGPU side going forward -- but "recommended entry point" language belongs to WebGL for now, not WebGPU.

The examples below use `WebGLRenderer` with classic `ShaderMaterial`/GLSL because that surface is stable, everywhere, and what most existing production code (and this skill's shader examples) is built on. It's also not a style choice you can defer: `WebGPURenderer` does not support `ShaderMaterial` or `onBeforeCompile()` at all -- projects on it author materials with TSL (Three.js Shading Language) node graphs instead of raw GLSL strings, a different authoring model with no drop-in bridge. **[Unverified]** the exact current TSL node syntax and migration path aren't reproduced here; check `threejs.org/docs/pages/WebGPURenderer.html` and the TSL docs directly before starting a WebGPU-first project, since that surface is still moving fast.
