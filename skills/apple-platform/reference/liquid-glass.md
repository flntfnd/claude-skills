...
Apps could opt out of Liquid Glass entirely on iOS 26 with the `UIDesignRequiresCompatibility` Info.plist key (set to `YES` to render the legacy pre-Liquid-Glass design). Apple always described this as temporary. Confirmed: Apple's own `UIDesignRequiresCompatibility` documentation, updated with the iOS 27 Beta release notes (June 9, 2026), states the system ignores the key once an app is built for iOS 27, iPadOS 27, Mac Catalyst 27, macOS 27, or tvOS 27 or later -- an app compiled against the iOS 27 SDK with the flag set to `YES` renders with Liquid Glass exactly as if the flag weren't present. There is no compatibility mode in Xcode 27. See `whats-new-ios27.md` for the full beta-status picture as of August 2026.
