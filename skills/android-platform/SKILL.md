...
That BOM (2026.08.00) maps to `material3` 1.4.0 stable. `material3` 1.5.0 is on the alpha track as of August 2026 and is where several remaining M3E component APIs (exact set unconfirmed — see below) are being promoted out of experimental; check the BOM mapping page before pinning a version.

Core M3E theming (`MaterialExpressiveTheme`, `expressiveLightColorScheme`/`expressiveDarkColorScheme`, `MotionScheme`) is stable and needs no opt-in as of `material3` 1.4.0. Individual newer components still commonly ship behind `@OptIn(ExperimentalMaterial3ExpressiveApi::class)` — **`[Unverified]`: the exact per-component stability boundary is inconsistent across sources as of August 2026 (some describe `LoadingIndicator`, `ButtonGroup`, `SplitButton`, and `FloatingToolbar` as already stable in 1.4.0; others describe them as still promoting through the 1.5.0 alpha train). Keep the `@OptIn` annotation in new code — it's harmless if the API turns out to already be stable — but verify against the current `compose-material3` release notes before removing it or shipping.**
