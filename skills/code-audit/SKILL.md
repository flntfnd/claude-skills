---
name: code-audit
description: Runs structured audits across seven categories -- general code quality, UI/design-system coherence, real-time audio thread safety, auth/API/data-layer security, Figma or Sketch design file handoff readiness, and the Rust toolchain (clippy, cargo audit, cargo deny, cargo test). Groups findings by category and flags each as critical, warning, or minor without rewriting anything unless asked. Use when the user asks for a code review, audit, security review, design file handoff check, pre-merge sweep, or wants dead code, memory leaks, hardcoded design values, RLS/JWT gaps, audio-thread allocations, or design token drift found and reported. For deep Rust-specific audit guidance (unwrap(), thiserror/anyhow conventions, unsafe block safety comments, clone() misuse), pair this with the rust-conventions skill's extended audit section rather than expecting Rust depth here.
---

# Audit

Run all applicable audits in order. Group findings by category. Flag every issue as critical, warning, or minor. Don't rewrite anything unless asked.

Pick the categories relevant to what's being audited -- a UI PR doesn't need the Audio Thread category, an auth change doesn't need the Design Files category. When the work touches Rust, also consult the rust-conventions skill: it carries a deeper, Rust-specific audit section (unwrap(), thiserror vs anyhow, clone() misuse, unsafe safety comments, manual index loops) that supersedes doing that analysis from scratch here.
...
Run these tools, not just manual inspection. For the accompanying manual-review checklist (unwrap(), error types, unsafe blocks, clones, iterator usage), see the rust-conventions skill -- it owns that depth now so it isn't duplicated here.
